			...
			<div class="line-dghp">
				...
				<div class="line-dghp-center">
					...
					@if($value_tpdg_child->id_baidanhgia == $value_tpdg->id)
						...
						@if($value_tpdg_child->has_id_bdg == 0) 
						...
						<div class="line-phuong-phap-danh-gia" id="list-danh-gia-hoc-phan-{{$value_tpdg_child->id}}" data-value="{{$value_tpdg->id}}">
							...
							<!-- <span class="line-ppdg-center" id="id-line-ppdg-center-{{$value_tpdg_child->id}}">
								<input type="hidden" name="bai_danh_gia[]" value="{{$value_tpdg_child->id}}">
								<span class="phuong-phap-danh-gia">
									<textarea name="bai_danh_gia[]" required="true" rows="5" cols="20"></textarea>
								</span>
								<span>
									<textarea name="bai_danh_gia[]" required="true" rows="5" cols="20"></textarea>
								</span>
								...
								<input type="hidden" name="bai_danh_gia[]" value="2">
						...
						@else
							...
							@foreach($all_danhgiahocphan as $vl_all_dghp)

								@if($vl_all_dghp->id_baidanhgia == $value_tpdg_child->id)
								...
								<div class="line-phuong-phap-danh-gia" id="list-danh-gia-hoc-phan-{{$value_tpdg_child->id}}" data-value="{{$value_tpdg->id}}">
									...
									<span class="line-ppdg-center" id="id-line-ppdg-center-{{$value_tpdg_child->id}}">
										<input type="hidden" name="bai_danh_gia[]" value="{{$value_tpdg_child->id}}">
										<span class="phuong-phap-danh-gia">
											<textarea name="bai_danh_gia[]" required="true" rows="5" cols="20">{{$vl_all_dghp->phuongphapdanhgia}}</textarea>
										</span>
										<span>
											<textarea name="bai_danh_gia[]" required="true" rows="5" cols="20">{{$vl_all_dghp->trongsobaidanhgia}}</textarea>
										</span>
										<span>
											<textarea name="bai_danh_gia[]" required="true" rows="5" cols="20">{{$vl_all_dghp->cdr_hocphan}}</textarea>
										</span>
								...
								<div class="action-line-dghp">
									<div id="add-line-ppdg-{{$value_tpdg_child->id}}" style="display:none;">
										<p class="action" id="add-phuong-phap-danh-gia" data-value="{{$value_tpdg_child->id}}">Thêm</p>
									</div>
									<div id="delete-line-ppdg-{{$value_tpdg_child->id}}">
										<p class="action" id="delete-phuong-phap-danh-gia" data-value="{{$value_tpdg_child->id}}">Xóa</p>
									</div>
								</div>
				...
				@if($value_tpdg->has_id_tstp == 0) 
					<span class="trong-so-thanh-phan">
						<textarea name="trong_so_thanh_phan[]" id="" cols="20" rows="3" required></textarea>
					</span>	
				@else

					@foreach($dghp_trongsothanhphan as $vl_dghp_tstp)
					@if($value_tpdg->id == $vl_dghp_tstp->id_baidanhgia_parent)
					<span class="trong-so-thanh-phan">
						<textarea name="trong_so_thanh_phan[]" id="" cols="20" rows="3" required>{{$vl_dghp_tstp->trongsothanhphan}}</textarea>
					</span>
					@endif
					@endforeach
